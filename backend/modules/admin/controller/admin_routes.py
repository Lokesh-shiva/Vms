...
def get_match_metrics(
    ...
    total_terminal = db.query(func.count(Match.id)).filter(
        Match.status.in_(terminal_statuses)
    ).scalar() or 0
    no_show_count = db.query(func.count(Match.id)).filter(
        Match.status == "CANCELLED_NO_SHOW"
    ).scalar() or 0
    no_show_rate = round(no_show_count / total_terminal * 100, 1) if total_terminal else 0.0
...
@router.get("/config")
def list_system_configs(current_user: dict = Depends(require_admin)):
