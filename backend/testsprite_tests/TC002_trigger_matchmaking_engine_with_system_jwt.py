...
def test_trigger_matchmaking_engine_with_system_jwt():
    ...
    assert "matches_created" in data or "matchesCreated" in data or "matches" in data, (
        "Response missing matches_created key"
    )
    assert "entries_updated" in data or "entriesUpdated" in data or "entries" in data, (
        "Response missing entries_updated key"
    )
    # The list of matched entry_ids might be under 'matched_entry_ids' or similar
    matched_keys = [
        k
        for k in data.keys()
        if "match" in k.lower() and ("entry_ids" in k.lower() or "entries" in k.lower())
    ]
