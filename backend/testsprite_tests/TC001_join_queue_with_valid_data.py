...
def test_join_queue_with_valid_data():
    ...
    def leave_queue():
        leave_resp = session.delete(
            BASE_URL + LEAVE_QUEUE_ENDPOINT,
            headers=headers,
            timeout=timeout
        )
        # Leave may fail if no active queue entry, ignore in cleanup
        if leave_resp.status_code not in (200, 400):
            raise Exception(f"Error during cleanup leave_queue: {leave_resp.status_code} {leave_resp.text}")
    ...
    try:
        ...
        assert join_resp.status_code == 201, f"Expected 201 Created but got {join_resp.status_code}: {join_resp.text}"
        ...
        # Validate values
        assert resp_data["sport_id"] == join_payload["sport_id"], "sport_id mismatch in response"
        assert resp_data["skill_level"] == join_payload["skill_level"], "skill_level mismatch in response"
        assert resp_data["status"] == "WAITING", f"Expected status 'WAITING' but got '{resp_data['status']}'"
        assert isinstance(resp_data["players_searching"], int) and resp_data["players_searching"] >= 0, "Invalid players_searching value"
        assert isinstance(resp_data["estimated_wait_seconds"], int) and resp_data["estimated_wait_seconds"] >= 0, "Invalid estimated_wait_seconds value"
        ...
        pricing_fields = {"base_price", "time_factor", "demand_factor", "queue_count", "final_price"}
        missing_pricing_fields = pricing_fields - pricing.keys()
        assert not missing_pricing_fields, f"Pricing missing fields: {missing_pricing_fields}"
