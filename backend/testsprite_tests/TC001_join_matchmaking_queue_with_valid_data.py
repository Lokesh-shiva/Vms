...
def test_join_matchmaking_queue_with_valid_data():
    ...
    try:
        join_resp = requests.post(
            join_url, json=join_payload, headers=headers, timeout=TIMEOUT
        )
        assert join_resp.status_code == 201, (
            f"Failed to join matchmaking queue: {join_resp.status_code} {join_resp.text}"
        )
        ...
        assert "estimated_wait_seconds" in join_data, (
            "Response missing estimated_wait_seconds"
        )
    ...
    if entry_id:
        ...
        try:
            ...
            assert leave_resp.status_code == 200, (
                f"Failed to leave matchmaking queue: {leave_resp.status_code} {leave_resp.text}"
            )
            leave_data = leave_resp.json()
            assert "status" in leave_data and leave_data["status"] == "CANCELLED", (
                "Queue leave did not cancel the entry"
            )
        except requests.RequestException as e:
