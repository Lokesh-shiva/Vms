...
def test_join_queue_with_duplicate_waiting_entry_should_fail():
    ...
    login_resp = requests.post(LOGIN_URL, json=test_user_credentials, headers=headers, timeout=TIMEOUT)
    ...
    try:
        ...
        first_resp = requests.post(PLAY_NOW_URL, json=join_data, headers=auth_headers, timeout=TIMEOUT)
        assert first_resp.status_code == 201, f"First join_queue failed: {first_resp.text}"
        ...
        second_resp = requests.post(PLAY_NOW_URL, json=join_data, headers=auth_headers, timeout=TIMEOUT)
        assert second_resp.status_code == 400, f"Duplicate join did not fail as expected: {second_resp.text}"
        err_message = second_resp.json().get("message") or ""
        expected_message = f"User {user_id} is already in the queue."
        assert expected_message == err_message, f"Unexpected error message: {err_message}"
        ...
        if leave_resp.status_code == 200:
            ...
            raise AssertionError(f"Failed to clean up queue entry: {leave_resp.status_code} {leave_resp.text}")
