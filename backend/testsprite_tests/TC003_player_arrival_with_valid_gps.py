...
def test_player_arrival_with_valid_gps():
    ...
    login_payload = {"username": unique_username, "password": "TestPass123!"}
    ...
    try:
        ...
        r = session.post(
            PLAY_NOW_URL, headers=headers, json=play_now_payload, timeout=TIMEOUT
        )
        ...
        matched_ids = (
            engine_response.get("matched_entry_ids")
            or engine_response.get("entry_ids")
            or []
        )
        ...
        for _ in range(max_poll_attempts):
            ...
            time.sleep(1)
        assert match_id, (
            "Failed to get match_id with status MATCHED or ARRIVED after matchmaking engine trigger"
        )
        ...
        r = session.post(
            arrive_url, headers=headers, json=arrive_payload, timeout=TIMEOUT
        )
        ...
        state = (
            ...
            or match_obj.get("match_status")
        )
        assert state in ["ARRIVED", "IN_PROGRESS"], (
            f"Unexpected match state after arrival: {state}"
        )
