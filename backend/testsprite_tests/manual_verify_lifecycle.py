...
def run_test():
    ...
    if None in tokens: return

    headers = [{"Authorization": f"Bearer {t}", "Content-Type": "application/json"} for t in tokens]

    print("\n[Step 1] Users joining queue...")
    payload = {"sport_id": 1, "skill_level": "BEGINNER"}
    
    for i, h in enumerate(headers):
        r = requests.post(f"{BASE_URL}/api/v1/matchmaking/play-now", headers=h, json=payload)
    ...
    r = requests.post(f"{BASE_URL}/engine/trigger", headers={"X-Cron-Secret": CRON_SECRET})
    ...
    r = requests.post(f"{BASE_URL}/api/v1/matches/{match_id}/arrive", 
                     headers=headers[0], 
    ...
    r = requests.post(f"{BASE_URL}/api/v1/matches/{match_id}/finish", headers=headers[0])
    if r.status_code != 200:
