...
def test_get_queue_status_with_active_entry():
    ...
    try:
        ...
        login_resp = requests.post(
            login_url, json={"phone": phone, "password": password}, timeout=TIMEOUT
        )
        ...
        join_resp = requests.post(
            play_now_url, headers=headers, json=join_payload, timeout=TIMEOUT
        )
        ...
        if join_resp.status_code == 201:
            ...
            assert "players_searching" in data, (
                "players_searching missing in join response"
            )
            assert "estimated_wait_seconds" in data, (
                "estimated_wait_seconds missing in join response"
            )
            assert "pricing" in data and isinstance(data["pricing"], dict), (
                "pricing missing or invalid in join response"
            )
        elif join_resp.status_code == 400:
            message = join_resp.json().get("message", "")
            assert "already in the queue" in message, (
                "Unexpected message when joining with active entry"
            )
        else:
            assert False, (
                f"Unexpected status code from play-now: {join_resp.status_code} Body: {join_resp.text}"
            )

        # Get queue status
        status_resp = requests.get(status_url, headers=headers, timeout=TIMEOUT)
        assert status_resp.status_code == 200, (
            f"Queue status retrieval failed: {status_resp.text}"
        )
        status_data = status_resp.json()
        data = (
            status_data.get("data") or status_data
        )  # accomodate if data is nested or flat
        assert data.get("status") == "WAITING", "Queue status is not WAITING"
        assert isinstance(data.get("players_searching"), int), (
            "players_searching missing or not int"
        )
        assert isinstance(data.get("estimated_wait_seconds"), int), (
            "estimated_wait_seconds missing or not int"
        )
        pricing = data.get("pricing")
        assert pricing is not None and isinstance(pricing, dict), (
            "pricing missing or invalid in status response"
        )
