...
def test_leave_queue_with_active_waiting_entry():
    ...
    assert login_resp.status_code == 200, (
        f"Login failed with status {login_resp.status_code}, body: {login_resp.text}"
    )
    ...
    play_now_payload = {"sport_id": SPORT_ID, "skill_level": SKILL_LEVEL}
    # Try to join the queue - must succeed with 201 or fail if user already in queue
    play_now_resp = requests.post(
    ...
    if play_now_resp.status_code == 400:
        ...
        assert play_now_resp.status_code == 201, (
            f"Join queue failed with status {play_now_resp.status_code}, body: {play_now_resp.text}"
        )
    ...
    assert leave_resp.status_code == 200, (
        f"Leave queue failed with status {leave_resp.status_code}, body: {leave_resp.text}"
    )
    ...
    assert data.get("status") == "CANCELLED", (
        f"Expected status CANCELLED but got {data.get('status')}"
    )
    assert "You have left the queue." in message, (
        f"Expected confirmation message not found, got: {message}"
    )
