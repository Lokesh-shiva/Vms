...
def test_join_queue_with_invalid_skill_level_should_fail():
    ...
    token = response.json().get("access_token") or response.json().get("token") or response.json().get("data", {}).get("token")
    ...
    response = requests.post(play_now_url, headers=headers, json=payload, timeout=TIMEOUT)
    ...
    expected_message = "Invalid skill_level. Must be one of BEGINNER|INTERMEDIATE|ADVANCED."
